...
$download_url = array(
'1' => 'http://creativeuniverse.tokyo/downloads/files/Luminance_Pack_V1.zip',
'2' => 'http://creativeuniverse.tokyo/downloads/files/cg_words.xlsx',
'3' => 'http://creativeuniverse.tokyo/downloads/files/cgl1.pptx'
);
...
$file_name = "footageCounter.php";

//ダウンロード履歴閲覧のための認証用ID、パスワード　※必ず変更して下さい！
//半角英数字のみ
$userid   = 'kodai';   // ユーザーID
$password = 'kodai462212';   // パスワード
...
$copyrights = '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="http://creativeuniverse.tokyo" target="_blank">- CreativeUniverse -</a>';
...
if(isset($_GET['dsp_count'])){
 ...
 //出力
$count_dsp = <<<EOF
document.write('<p class="download_count">Total Downloads：{$total}（<span class="count_today">Today:{$today_count}</span> <span class="count_yesterday">Yesterday:{$yesterday_count}</span>）</p>')
EOF;

	}else{
//出力
$count_dsp = <<<EOF
document.write('<h1 class="download_count">Total Downloads：{$total}</h1>')
EOF;
	}
	//UTF-8以外であれば文字コード変更
