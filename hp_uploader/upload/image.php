<?php
  ...
  //フォーム入力有無確認
  if(!empty($_POST["name"]) && !empty($_POST["categories"]) && !empty($_FILES["file"]["name"]) && !empty($_POST["regist"])){
      start($mysqli);
  } else{
  ...
  function start($mysqli){
    ...
    try{
      ...
      try{
        ...
        $sql_add = "INSERT INTO image( name, detail, category, reg_date) ";
        $sql_add .= "VALUES( ";
        $sql_add .= " '".htmlspecialchars($_POST["name"])."', ";
        $sql_add .= " '".htmlspecialchars($_POST["detail"])."', ";
        ...
        try{
          ...
          if(is_uploaded_file($_FILES["file"]["tmp_name"])) {
            ...
            if($width >= $min_width || $height >= $min_height){
              ...
              if (!file_exists("../images/img/thumbnail/" . $next_id . ".jpg")) {
                ImageJPEG($out, "../images/img/thumbnail/" . $next_id . ".jpg");
              } else{
                throw new Exception("サムネイル : 同名のファイルが存在します");
              }
          ...
          if (!file_exists("../images/img/large/" . $next_id . ".jpg")) {
            // Uploading
            if (move_uploaded_file($_FILES["file"]["tmp_name"], "../images/img/large/" . $next_id  . ".jpg")) {
              //アップロード成功。トップへリダイレクト。
            ...
            throw new Exception("メイン画像 : 同名のファイルが存在します");
          }

        } catch(Exception $e){
            //jpegアップロード失敗した際、データベースから消去
            if(mysqli_query( $mysqli, "DELETE FROM image WHERE id = '" . $next_id . "'" )){
