<?php
  ...
  //フォーム入力有無確認
  if(!empty($_POST["name"]) && !empty($_POST["categories"]) && !empty($_POST["date"]) && !empty($_FILES["file"]["name"]) && !empty($_POST["regist"])){
      start($mysqli);
  } else{
  ...
  function start($mysqli){
    ...
    try{
      ...
      try{
        ...
        $sql_add = "INSERT INTO photograph( name, category, orig_date, reg_date) ";
        $sql_add .= "VALUES( ";
        ...
        $sql_add .= " '" . htmlspecialchars($_POST["date"]) ."', ";
        ...
        try{
          ...
          if(is_uploaded_file($_FILES["file"]["tmp_name"])) {
            ...
            if($width >= $min_width || $height >= $min_height){
              ...
              if (!file_exists("../photograph/img/thumbnail/" . $next_id . ".jpg")) {
                ImageJPEG($out, "../photograph/img/thumbnail/" . $next_id . ".jpg");
              } else{
          ...
          if (!file_exists("../photograph/img/large/" . $next_id . ".jpg")) {
            // Uploading
            if (move_uploaded_file($_FILES["file"]["tmp_name"], "../photograph/img/large/" . $next_id  . ".jpg")) {
              //アップロード成功。トップへリダイレクト。
              header("Location: http://".$_SERVER["HTTP_HOST"].dirname($_SERVER["SCRIPT_NAME"]));
            } else {
                throw new Exception("サイズオーバーによるアップロード失敗");
            }
          } else{
            throw new Exception("メイン画像 : 同名のファイルが存在します。サーバーをチェックしてください。");
          }

        } catch(Exception $e){
            //jpegアップロード失敗した際、データベースから消去
            if(mysqli_query( $mysqli, "DELETE FROM photograph WHERE id = '" . $next_id . "'" )){
