		...
		$(document).ready(function() {
			...
			$(settings.menu_contents).bind("touchmove.scrollMenu", function() {
				...
				$(this).css({
					...
					'-webkit-transform': 'translate3d(0px,'+ moveY +'px,30px)'
				});
			});
			...
			$(settings.menu_contents).bind("touchend.scrollMenu", function() {
				...
				if (moveY > 0) {
					$(settings.menu_contents).css({
						...
						'-webkit-transform': 'translate3d(0px,'+ 0 +'px,30px)'
					});
					smY = 0;
				} else if (screen.height - menu_list_height > moveY + settings.bottom_margin) {
					$(settings.menu_contents).css({
						'-webkit-transition': '-webkit-transform .5s ease-out',
						'-webkit-transform': 'translate3d(0px,'+ (screen.height - menu_list_height - settings.bottom_margin) +'px,30px)'
					});
